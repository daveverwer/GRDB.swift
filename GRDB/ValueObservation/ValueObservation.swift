...
extension ValueObservation: Refinable {
    ...
    /// Fetches the initial value.
    func fetchInitialValue(_ snapshot: DatabaseSnapshot) throws -> Reducer.Value
    where Reducer: SnapshotReducer
    {
        var reducer = makeReducer()
        guard let value = try reducer._value(reducer._fetch(snapshot)) else {
            fatalError("Broken contract: reducer has no initial value")
        }
        return value
    }
...
extension ValueObservation where Reducer == ValueReducers.Auto {
    ...
    -> ValueObservation<ValueReducers.SnapshotFetch<Value>>
    {
        .init(
            trackingMode: .constantRegionRecordedFromSelection,
            makeReducer: {
                ValueReducers.SnapshotFetch { snapshot in
                    return try snapshot.unsafeReentrantRead { db in
